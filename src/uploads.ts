 ...
 *
 * This is only for uploading files that the app happens to already have
 * somehow, e.g. generated by the app or downloaded from elsewhere. For a file
 * coming from a user's browser, call `uploadFile` from
 * `@collegevine/trellis-app-sdk/client` instead, then {@link adoptFile} to get
 * an id for this call. That path works differently, via S3 pre-signed URLs, in
 * order to work around AWS Lambda's limit on request size.
 *
 ...
 * `invalid_file` (400, empty body or missing filename), `invalid_upload_ids`
 * (400). Referencing an unknown id at inference time throws `upload_not_found`
 * (400).
 *
 * @example
 * ```ts
 * const { upload_id } = await uploadFile(bytes, "application/pdf",
 * "report.pdf")
